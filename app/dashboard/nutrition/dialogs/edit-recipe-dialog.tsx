...
function SortableIngredient({ ing, isDark, onGramsChange, onRemove, flashIngId }: {
...
export default function EditRecipeDialog({ recipe, open, onClose, onSuccess }: Props) {
  ...
  const fetchFoods = async () => {
    ...
    const q = uid
      ? supabase.from('foods').select('id,name,category,calories_per_100g,protein_per_100g,carbs_per_100g,fat_per_100g,is_default,trainer_id,extras').or(`trainer_id.eq.${uid},is_default.eq.true`).order('name').limit(2000)
      : supabase.from('foods').select('id,name,category,calories_per_100g,protein_per_100g,carbs_per_100g,fat_per_100g,is_default,trainer_id,extras').eq('is_default', true).order('name').limit(2000)
    const { data } = await q
    setFoodsLoaded(true)
            ...
            <div className="space-y-2">
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} modifiers={[restrictToVerticalAxis]} onDragEnd={reorderIngredients}>
                <SortableContext items={ingredients.map(i => i.food_id)} strategy={verticalListSortingStrategy}>
                  {ingredients.map(ing => (
                    <SortableIngredient key={ing.food_id} ing={ing} isDark={isDark} flashIngId={flashIngId}
