  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl flex flex-col max-h-[90vh] p-0 gap-0 overflow-hidden" showCloseButton={false}>
        <DialogTitle className="sr-only">{isClientEdit ? t('clientEditTitle', { planName: plan.name }) : t('editTitle')}</DialogTitle>
        <DialogDescription className="sr-only">{isClientEdit ? t('clientEditTitle', { planName: plan.name }) : t('editTitle')}</DialogDescription>
        ...
        <div className="bg-gradient-to-r from-purple-600 to-violet-500 px-6 py-4 shrink-0 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-white font-bold text-base truncate">
              {isClientEdit ? t('clientEditTitle', { planName: plan.name }) : t('editTitle')}
            </h2>
            <p className="text-purple-100/70 text-xs">
              {isClientEdit ? t('clientEditSubtitle') : t('editSubtitle')}
            </p>
          </div>
            ...
            <div className="px-6 pt-4 pb-4 space-y-3">
              {isClientEdit && (
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold">{t('clientPlanNameLabel')}</Label>
                  <Input
                    value={customName}
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  { key: 'calories', label: t('kcalLabel'),         placeholder: '2000' },
                  { key: 'protein',  label: t('proteinInputLabel'), placeholder: '150'  },
                  { key: 'carbs',    label: t('carbsInputLabel'),   placeholder: '200'  },
                  { key: 'fat',      label: t('fatInputLabel'),     placeholder: '70'   },
                ].map(f => (
          ...
          {meals.length > 0 && (
            <div className="rounded-md bg-gray-50 px-4 py-2.5 grid grid-cols-4 gap-2 text-center text-xs">
              {([
                ...
                { label: t('carbsSummaryLabel'),   val: Math.round(totals.carbs),    tgt: tgt.carbs,    unit: 'g' },
