...
export default function MobileFinanceView() {
  ...
  return (
    <div className="space-y-4 pb-4">
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-gray-900">Financije</h1>
        <p className="text-sm text-gray-400 mt-0.5">Pregled paketa i plaćanja</p>
      </div>
      ...
      <div className="rounded-3xl px-5 py-5 text-white overflow-hidden relative"
        ...
        <p className="text-white/60 text-xs font-medium uppercase tracking-wide">Ukupno fakturirano</p>
        <p className="text-white font-black text-3xl mt-1">{totalFakturirano.toLocaleString('hr-HR')} €</p>
        <div className="flex gap-6 mt-4">
          <div>
            <p className="text-white/60 text-xs">Naplaćeno</p>
            <p className="text-emerald-300 font-bold text-lg">{totalPaid.toLocaleString('hr-HR')} €</p>
          </div>
          <div>
            <p className="text-white/60 text-xs">Čeka na naplatu</p>
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {[
          { label: 'Ukupno',      value: items.length,   icon: Banknote,     color: accentHex  },
          { label: 'Plaćeno',     value: paid.length,    icon: CheckCircle2, color: '#16a34a'  },
          { label: 'Nenaplaćeno', value: unpaid.length,  icon: Clock,        color: unpaid.length > 0 ? '#d97706' : '#6b7280' },
        ].map(s => {
          const Icon = s.icon
          return (
      ...
      <div className="flex gap-2">
        {(['unpaid', 'all'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'unpaid' ? `Nenaplaćeno (${unpaid.length})` : `Sve (${items.length})`}
          </button>
        ))}
      </div>
      ...
      {displayed.length === 0 ? (
        <div className="flex items-center justify-center py-12 text-center">
          <div>
            ...
            <p className="text-sm font-semibold text-gray-500">Sve je naplaćeno!</p>
          </div>
        </div>
      ) : (
              ...
              <div key={p.id} className="flex items-center gap-3 px-4 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 truncate">
                    {p.packageName || 'Paket'}
                    {p.endDate && ` · do ${new Date(p.endDate).toLocaleDateString('hr-HR', { day: '2-digit', month: '2-digit' })}`}
                  </p>
                </div>
